<?php
	if(strpos($_SERVER['PHP_SELF'], "admin_utilities_log_recent")){
		echo "<h2>ERROR:  Cannot Load Logic Files Individually!</h2>";
	}

	$sql = sprintf("SELECT * FROM log WHERE email = 0 ORDER BY year, month, day, hour, minute DESC LIMIT 25");
		...
		while($row = mysql_fetch_array($result)){
			echo '<tr>';
				echo '<td align="center">';
					echo '<a href="admin_utilities_log.php?action=details&logid=' . $row['logid'] . '">' . $row['logid'] . '</a>';
				echo '</td>';
